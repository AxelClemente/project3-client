...
import { RiUserHeartLine, RiTimerLine, RiWalkFill, RiStarSFill} from 'react-icons/ri';
...
const StrollList = () => {
    ...
    const handleFavoritesClick = (strollId) => {
      ...
      const getAverageRating = (strollId) => {
        axios.get(`${API_URL}/rating/strolls/${strollId}/rating`)
          .then((response) => {
                ...
                <div className="mb-4">
                  <div className="text-sm mb-8 flex" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div className="flex gap-x-2" style={{ alignItems: 'center' }}>
                      <div><RiStarSFill/></div>
                      <p style={{ fontWeight: 'bold' }}>{averageRatings[stroll._id] || 0}</p>
                    </div>
                  </div>
